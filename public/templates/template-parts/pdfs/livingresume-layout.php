...
<article id="<?php echo the_ID(); ?>" <?php post_class(); ?>>
	...
	include living_resume_get_template( 'pdfs/livingresume-header-layout' );
	...
	foreach ( $living_resume_sort_order as $key => $value ) {
		if ( $key == '_living_resume_sort_introduction' ) {
			include living_resume_get_template( 'pdfs/livingresume-introduction' );
		}
		elseif ( $key == '_living_resume_sort_employment_history' ) {
			include living_resume_get_template( 'pdfs/livingresume-employment' );
		}
		elseif ( $key == '_living_resume_sort_education_history' ) {
			include living_resume_get_template( 'pdfs/livingresume-education' );
		}
		elseif ( $key == '_living_resume_sort_skills_block' ) {
			include living_resume_get_template( 'pdfs/livingresume-skills' );
		}
		elseif ( $key == '_living_resume_sort_tools_block' ) {
			include living_resume_get_template( 'pdfs/livingresume-tools' );
		}
		elseif ( $key == '_living_resume_sort_additional_one' ) {
			include living_resume_get_template( 'pdfs/livingresume-additional-one' );
		}
		elseif ( $key == '_living_resume_sort_additional_two' ) {
			include living_resume_get_template( 'pdfs/livingresume-additional-two' );
		}
	}

	/* Include resume footer */
	include living_resume_get_template( 'pdfs/livingresume-footer-layout' );
	?>
	</div>
</article>